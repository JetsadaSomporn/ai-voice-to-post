...
import { useAuth } from '@/conte        addResult(`🔓 Is Public: ${data.bucket.public ? 'YES' : 'NO'}`)
        addResult(`📁 Files count: ${data.filesCount}`)
        addResult(`📋 All buckets: ${data.allBuckets?.map((b: any) => `${b.name}(${b.public ? 'public' : 'private'})`).join(', ') || 'none'}`)/AuthContext'
...
export default function QuickStorageTest() {
  ...
  const checkBucketConfig = async () => {
    try {
      ...
      if (response.ok) {
        addResult(`✅ Storage API Response: ${JSON.stringify(data, null, 2)}`)
        addResult(`📦 Bucket found: ${data.bucket.name}`)
        addResult(`🔓 Is Public: ${data.bucket.public ? 'YES' : 'NO'}`)
        addResult(`� Files count: ${data.filesCount}`)
        addResult(`� All buckets: ${data.allBuckets.map(b => `${b.name}(${b.public ? 'public' : 'private'})`).join(', ')}`)
      } else {
